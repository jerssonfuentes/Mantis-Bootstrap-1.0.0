...
// Función para obtener y mostrar el valor del dólar
function actualizarValorDolar() {
    fetch("https://api.exchangerate-api.com/v4/latest/USD")
        ...
        .then(data => {
            ...
            // Busca el elemento donde se muestra el precio del dólar
            const dolarElements = document.querySelectorAll('.dolar-price');
            if (dolarElements.length > 0) {
                dolarElements.forEach(element => {
                    element.textContent = `${copRate.toFixed(2)} COP`;
                });
            }
        })
        .catch(error => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Actualizar el valor del dólar al cargar la página
    actualizarValorDolar();

    // Actualizar cada hora (opcional)
    setInterval(actualizarValorDolar, 3600000);
